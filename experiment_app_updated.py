...
GA_MEASUREMENT_ID = st.secrets.get("GA_MEASUREMENT_ID")  # optional

N_TRUE, N_FALSE, N_PHOTO_EACH = 8, 8, 4  # experiment parameters
...
def create_subset(pool: List[Stimulus]) -> List[Stimulus]:
    ...
    for s in with_photo:
        if s.truth and t < N_PHOTO_EACH:
            s.show_photo = True; t += 1
        elif not s.truth and f < N_PHOTO_EACH:
            s.show_photo = True; f += 1
...
def run_trial(stim: Stimulus, i: int, total: int):
    ...
    with st.form(f"form_{stim.id}"):
        answer = st.radio("Is the statement…", ["True", "False"], horizontal=True)
        txt = st.text_area(
            "Explain:" if ss.prompt_group == "Explain" else "Feelings:",
            height=120
        )
        submitted = st.form_submit_button("Submit & Next", **BUTTON_OPTS)

    if submitted:
        if not txt.strip():
            st.warning("Text box cannot be empty.")
            st.stop()

        rt = round(time.time() - ss.start_time, 2)
        ss.start_time = None
...
def finish():
    ...
    if LOCAL_CSV.exists():
        df = pd.concat([pd.read_csv(LOCAL_CSV), df], ignore_index=True)
...
if ss.get("show_instructions", True):
    show_instructions()
elif ss.index < len(ss.stimuli):
    run_trial(ss.stimuli[ss.index], ss.index + 1, len(ss.stimuli))
else:
    finish()
